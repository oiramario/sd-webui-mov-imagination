...
def extract(frames_path, keyframes_path, thres):
    ...
    with tqdm(total=len(image_paths), unit='frame') as pbar:
    ...
    with tqdm(total=len(indices) + 2, unit='keyframe') as pbar:
        for x in indices:
            full_color[x].save(os.path.join(keyframes_path , f'{x:05d}.png'))
            pbar.update(1)
        full_color[0].save(os.path.join(keyframes_path , f'{1:05d}.png'))
        pbar.update(1)
